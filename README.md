...
if 'listening_inputs' not in globals() or 'points_listening' not in globals():
    raise RuntimeError("⚠️ Please run CELL 1 first to set up the widgets.")
...
def grade_image(path, all_scores_list, template_path):
    ...
    if des_img is None or template_des is None:
        ...
        if len(good_matches) >= 4:
            ...
            if M is not None:
                ...
                print(f"✅ Successfully aligned {os.path.basename(path)}")
    ...
    print(f"📝 {filename} graded: L={scores['listening']:.1f}, S={scores['structure']:.1f}, R={scores['reading']:.1f}")
...
def batch_grade(template_path):
    ...
    if all_scores_from_bubble:
        ...
        print(f"\n✅ CSV saved to: {BUBBLE_GRADING_CSV}")
    else:
